...
$(function($) {
  ...
  var $rotatePage = $("#rotate").hide();
  var $fullscreenPage = $("#fullscreen").hide();
  var $requestPage = screen.orientation ? $fullscreenPage : $rotatePage;
  ...
  $("#go-fullscreen").on("tap", function(e) {
    try {
      var docElm = document.documentElement;
      var requestFullscreen =
        docElm.requestFullscreen || docElm.mozRequestFullScreen || docElm.webkitRequestFullScreen;
      // alert(requestFullscreen);
      if (requestFullscreen) {
        requestFullscreen.apply(docElm);
      }
  ...
  //******** Click Events ********//
  $("#messages-toggle").on("tap", function(e) {
    $("#messages").toggle().scrollTop(100000);
  });

  $("#leaveButton").on("tap", function(e) {
    e.preventDefault();
  ...
  $("#newGameButton").on("tap", function(e) {
    e.preventDefault();
  ...
  $doc.on("tap", "#joinGameButton", function(e) {
    e.preventDefault();
    var id = $(this).data("id");
    if (id) {
  ...
  $("#startGameButton").on("tap", function(e) {
    e.preventDefault();
    var user = main.getCurrentUser();
    var game = main.getCurrentUserGame();
    if (user && game) {
  ...
  $("#readyGameButton").on("tap", function(e) {
    e.preventDefault();
    var game = main.getCurrentUserGame();
    if (game) {
  ...
  $("#playGameButton").on("tap", function(e) {
    e.preventDefault();
    var cards = $("#playingInput .card").filter(dataSelected);
  ...
  $("#drawGameButton").on("tap", function(e) {
    e.preventDefault();
    var game = main.getCurrentUserGame();
  ...
  $("#giveCardButton").on("tap", function(e) {
    e.preventDefault();
    var game = main.getCurrentUserGame();
    var from = main.gameData.favor.from;
    if (game && from) {
      //Get the selected card id
      var cards = $("#givePopup .card").filter(dataSelected);
  ...
  $("#favorSelectButton").on("tap", function(e) {
    e.preventDefault();
    var cards = $("#playingInput .card").filter(dataSelected);
  ...
  $("#blindStealButton").on("tap", function(e) {
    e.preventDefault();
    var cards = $("#playingInput .card").filter(dataSelected);
  ...
  $("#namedStealButton").on("tap", function(e) {
    e.preventDefault();
    var cards = $("#playingInput .card").filter(dataSelected);
    var to = $("#namedStealPopup #player-select").val();
    var cardTypes = $("#namedStealPopup .card").filter(dataSelected);
    var game = main.getCurrentUserGame();
  ...
  $("#discardStealButton").on("tap", function(e) {
    e.preventDefault();
    var cards = $("#playingInput .card").filter(dataSelected);
    var discardCards = $("#discardStealPopup .card").filter(dataSelected);
    var game = main.getCurrentUserGame();
  ...
  $("#nopeGameButton").on("tap", function(e) {
    e.preventDefault();
    var game = main.getCurrentUserGame();
    ...
    if (game && main.gameData.hasCardTypeInHand($C.CARD.NOPE) && currentSet) {
      ...
      if (
        currentSet.owner.user.id === main.getCurrentUser().id &&
        main.gameData.currentPlayedSet.nopeAmount % 2 == 0
      ) {
  ...
  //Card tap
  $doc
    .on("tap", "#playingInput .card", function(e) {
      e.preventDefault();
      toggleCardSelected($(this));
      GameRoom.updateInputDisplay(main);
    })
    .on("tap", ".popup .card", function(e) {
      e.preventDefault();
      $(".popup .card")
        .not(this)
        .data("selected", "false")
  ...
  io.on($C.GAME.PLAYER.STEAL, function(data) {
    ...
    switch (data.type) {
      ...
      case $C.CARDSET.STEAL.NAMED:
        ...
        if (data.success) {
          GameRoom.logSystemGreen(
            fromString + " took a " + data.cardType + " from " + toString + "."
          );
        } else {
          GameRoom.logSystemGreen(
            fromString + " failed to take a " + data.cardType + " from " + toString + "."
          );
        }
        break;
      case $C.CARDSET.STEAL.DISCARD:
        ...
        if (data.success) {
          GameRoom.logSystemGreen(
            fromString + " took a " + data.card.name + " from the discard pile."
          );
        } else {
          GameRoom.logSystemGreen(
            fromString + " failed to take a " + data.card.name + " from  the discard pile."
          );
        }
        break;
    }
  ...
  var startNopeTimer = function(time) {
    ...
    var $nopeGameButton = $("#nopeGameButton");
    $nopeGameButton.data("count", Math.floor(time / 1000));
    updateNopeButton();
    if ($nopeGameButton.data("timer-started") == 1) return;

    var interval = setInterval(function() {
      var count = $nopeGameButton.data("count");
      $nopeGameButton.data("timer-started", 1);

      count--;
      $nopeGameButton.data("count", count);
 ...
 */
  ...
  var toggleCardSelected = function(card) {
    ...
    var selected = card.data("selected") === "true";
    selected = !selected;
    if (selected) {
    ...
    card.data("selected", selected.toString());
  };
  ...
  var gameFromData = function(data) {
    ...
    $.each(data.players, function(index, player) {
      ...
      if (user) {
        players.push(
          new Player(user.id, player.alive, player.ready, player.drawAmount, player.cardCount)
        );
      }
    });
