  ...
  updateGameList: function(EK) {
    ...
    $.each(EK.games, function(id, game) {
      ...
      var html =
        ...
        "</div>" +
        "<div id='bottom'>" +
        "<button id='joinGameButton' data-id='" +
        game.id +
...
var GameRoom = {
    ...
    if (user && game) {
      ...
      if (game.status === $C.GAME.STATUS.WAITING) {
        ...
        if (currentPlayer && currentPlayer.user === user.id) {
          ...
          if ($("#playingInput .card").filter(dataSelected).length < 1) {
            drawButton.show();
            playButton.hide();
          } else {
            playButton.show();
  ...
  updateCardDisplay: function(EK) {
    ...
    $.each($C.CARD, function(key, type) {
      if (type != $C.CARD.EXPLODE) {
        var html =
          "<div data-selected='false' data-type='" +
          type +
  ...
  updatePlayerList: function(EK) {
    if (EK.getCurrentUser() && EK.getCurrentUserGame()) {
      ...
      //Set the user count
      $("#playerList .top-bar").text("Players (" + game.players.length + ")");
    }
  },
  ...
  shouldEnablePlayButton: function(EK) {
    ...
    var ids = $("#playingInput .card").filter(function(index) {
      return $(this).data("selected") === "true";
    });
    ...
    switch (cards.length) {
      case 1: //Don't allow playing defuse, regular, nope or explode (if it somehow got into players hand)
        return !(
          cards[0].type === $C.CARD.DEFUSE ||
          cards[0].type === $C.CARD.EXPLODE ||
          cards[0].type === $C.CARD.REGULAR ||
          cards[0].type === $C.CARD.NOPE
        );
      case 2: //Blind pick
      case 3: //Named pick
